...
const HotelInfo = () => {
    ...
    useEffect(() => {
        axios(`http://localhost:5000/hotels/search?cityName=${sendData?.cityName}`)
            .then(res => {
                setSearchData(false)
    ...
    return (
        <div>
            <div className="row mb-5">
                {
                    searchData ?
                        ...
                        hotel.length === 0 ?
                            ...
                            :
                            hotel?.slice(pageCount*6,pageCount*6+6).map(data => <FlightOption data={data} key={data._id} />)
            ...
            <ReactPaginate
                    ...
                    nextLabel={"next"}
                    breakLabel={"..."}
                    pageCount={Math.ceil(hotel.length/6)}
