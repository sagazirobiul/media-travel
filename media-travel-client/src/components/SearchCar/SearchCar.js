...
const SearchCar = () => {
    ...
    const onSubmit = (data) => {
        if(data.carFrom === data.carTo){
            toast.error('Please do a valid query. From and To should be unique.!');
        } else {
            setSendData(data)
        }
    }
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
            <Row>
                <Col md={4}>
                    <Row>
                        <Col>
                            <Form.Label className="fw-bold">Pick Up:</Form.Label>
                            <Form.Select defaultValue="Canada" {...register("carFrom", { required: true })}>
                        ...
                        <Col>
                            ...
                            <Form.Select defaultValue="NewYork" {...register("carTo", { required: true })}>
                                <option>Canada</option>
                                <option>Alaska</option>
                                <option>Bahamas</option>
                                <option>California</option>
